...
class Radio(commands.Cog):
    ...
    async def _play_state(self, ctx: discord.ApplicationContext, option: Option(str, "Resume or Pause",
                                                                                autocomplete=basic_autocomplete(
                                                                                    play_state_autocomplete))):
        if option == "pause":
            ...
            return await ctx.respond("Das Radio hat irgendeinen Fehler schreibe in #bugs-und-report diesen Fehler",
                                     ephemeral=True)
    ...
    async def _join(self, ctx: discord.ApplicationContext):
        if not ctx.author.voice:
            return await ctx.respond(
                "Wo soll ich spielen wenn du nicht in einem Voice Channel bist? <:pepemcs:866283900619063307> ",
                ephemeral=True)
    ...
    async def _stop(self, ctx: discord.ApplicationContext):
        if not ctx.author.voice:
            return await ctx.respond(
                "Wie soll ich aufhören zu spielen wenn du nichtmal in einem Voice Channel bist? <:pepemcs:866283900619063307> ",
                ephemeral=True)
    ...
    async def _leave(self, ctx: discord.ApplicationContext):
        if not ctx.author.voice:
            return await ctx.respond(
                "Woher soll ich wissen aus welchen Voice Channel ich leaven soll wenn du nichtmal in einem Voice Channel bist? <:pepemcs:866283900619063307> ",
                ephemeral=True)
    ...
    async def _play(self, ctx: discord.ApplicationContext,
                    radio: Option(str, "Suche dir ein Radio aus", autocomplete=basic_autocomplete(radio_player))):
